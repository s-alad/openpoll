          ...
          {activePolls.map((poll) => {
            return (
              <form
                key={poll.id}
                className={s.poll}
                onSubmit={handleSubmit((data) => submitPoll(data, poll.id))}
              >
                <h1>{poll.question}</h1>
                <div className={s.options}>
                  {poll.options.map(
                    (option: { option: string; letter: string }) => {
                      return (
                        <div key={option.letter} className={s.option}>
                          <Controller
                            ...
                            defaultValue={
                              poll.responses &&
                              poll.responses[option.letter] &&
                              user!.uid in poll.responses[option.letter]
                            }
                            render={({ field }) => (
                              <FormControlLabel
                                control={
